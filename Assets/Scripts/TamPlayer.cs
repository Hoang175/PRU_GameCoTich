...
//        // --- THÊM DÒNG NÀY ---
//        // Gọi hàm thử nhặt cá khi nhấn Space
//        GetComponent<FishCollector>()?.TryPickUpFish();
...
public class Tam : MonoBehaviour
{
    ...
    void StartPick()
    {
        ...
        // Gọi hàm kiểm tra tương tác bên FishCollector
        GetComponent<FishCollector>()?.TryInteract();

        // Chạy Animation tương ứng với hướng đang đứng
        switch (facing)
        {
            case FacingDirection.Left:
            case FacingDirection.Right:
        ...
        // PHÒNG THỦ: Tránh lỗi kẹt Animation nếu quên set Animation Event
        Invoke("EndPick", 0.5f);
    }

    // Hàm này được gọi bởi Animation Event ở frame cuối của Clip Pick
    public void EndPick()
    {
        isPicking = false;
        animator.ResetTrigger("pickSide");
